...
// Get devnet connection with fallback
async function getDevnetConnection(): Promise<Connection> {
  for (const endpoint of DEVNET_ENDPOINTS) {
    try {
      const connection = new Connection(endpoint.url, 'confirmed')
      // Test the connection
      await connection.getBlockHeight()
      console.log(`✅ Using Devnet RPC endpoint: ${endpoint.name}`)
      return connection
    } catch (error) {
  ...
  throw new Error('All devnet RPC endpoints failed')
}
...
// Create and send devnet transaction
async function createAndSendDevnetTransaction(
  ...
  try {
    ...
    console.log('🚀 Sending devnet transaction...')
    ...
    console.error('Devnet transaction failed:', error)
    throw error
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Execute devnet transaction
    const result = await createAndSendDevnetTransaction(fromPrivateKey, toAddress, amount)

    return NextResponse.json({
      success: true,
      message: 'Devnet transaction sent successfully',
      data: result
    }, { headers })
